...
def generate_metrics(genome: np.ndarray, culture: dict, rng: np.random.Generator,
    ...
    base = np.array([
        ...
        0.025 + 0.12 * novelty + 0.06 * eff_stress,                           # nav_back_rate — novelty drives more exploration
        max(0, 0.25 * (1 - eff_pace) * (1 + eff_stress * 0.2) + 0.10 * novelty), # idle_ratio — novelty seekers wander/idle more
        0.02 + 0.09 * eff_stress + 0.04 * (1 - adaptability),                   # repeated_click_ratio
        max(30, 160 + 280 * attention * (1 - 0.25 * eff_stress) * (1 - 0.12 * novelty)), # session_duration_s — novelty seekers bail sooner
    ], dtype=np.float32)
